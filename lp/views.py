...
from .models import Leads, Perfil, Tagmeta, TagGoogle, TagGoogleBody, LandingPage, Cores, ItensOpcioanis, QtdeQuartos, \
    Qtdesuites, QtdeVagaGaragem
...
def dashboard_lp(request):
    if request.method == 'POST':
        ...
        itens = ItensOpcioanis.objects.all()
        perfil_user = Perfil.objects.last()
        qtde_quartos = QtdeQuartos.objects.all()
        qtde_suite = Qtdesuites.objects.all()
        qtde_vagas_garagem = QtdeVagaGaragem.objects.all()
        return render(request, 'site/dashboard-lp.html', {'cores': cores,
                                                          'itens': itens,
                                                          'perfil_user': perfil_user,
                                                          'qtde_quartos': qtde_quartos,
                                                          'qtde_suites': qtde_suite,
                                                          'qtde_vagas_garagem': qtde_vagas_garagem})
